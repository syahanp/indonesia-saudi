...
export default function Proyek() {
  ...
  // Map filters
  const [mapProgram, setMapProgram] = useState('Semua Program');
  const [mapStatus, setMapStatus] = useState('Semua Status');
  ...
  const filteredGridProjects = useMemo(() => {
    return initialProjects.filter(p => {
      ...
      if (program !== 'Semua Program') {
        if (program === 'Jembatan Gantung' && p.type !== 'jembatan') return false;
        if (program === 'Listrik Masuk Desa' && p.type !== 'listrik') return false;
      }
      
      if (status !== 'Semua Status' && p.status !== status) return false;
      
      if (provinsi !== 'Semua Provinsi' && p.loc !== provinsi) return false;
      
      return true;
    });
  }, [search, program, status, provinsi]);

  const filteredMapProjects = useMemo(() => {
    return initialProjects.filter(p => {
      if (mapProgram !== 'Semua Program') {
        if (mapProgram === 'Jembatan Gantung' && p.type !== 'jembatan') return false;
        if (mapProgram === 'Listrik Masuk Desa' && p.type !== 'listrik') return false;
      }
      if (mapStatus !== 'Semua Status' && p.status !== mapStatus) return false;
      return true;
    });
  }, [mapProgram, mapStatus]);
    ...
    <div className="pt-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative z-20 -mt-20 lg:-mt-28 bg-white rounded-2xl border border-slate-200 p-6 lg:p-8 shadow-xl mb-12 flex flex-col gap-4">
          <h3 className="font-bold text-slate-900 text-[18px]">Filter Proyek</h3>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
              <label className="block text-[12px] font-bold text-slate-700 mb-2">Cari Proyek</label>
            ...
            <div>
              <label className="block text-[12px] font-bold text-slate-700 mb-2">Program</label>
              <div className="relative">
                <select 
                  value={program}
            ...
            <div>
              <label className="block text-[12px] font-bold text-slate-700 mb-2">Provinsi</label>
              <div className="relative">
                 <select 
                  value={provinsi}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <p className="text-[13px] text-slate-600 font-semibold shrink-0">
            {view === 'grid' ? filteredGridProjects.length : filteredMapProjects.length} proyek ditemukan
          </p>
                  ...
                  <div className="p-6 flex flex-col flex-1 pt-8">
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100 mt-auto">
                      ...
                      <div className="pt-3 border-t border-slate-100">
                        <Link to="/proyek/jembatan-desa-lembah-harapan" className="relative z-20 flex items-center justify-between w-full text-[13px] font-bold text-slate-800 hover:text-primary-deep group/btn transition-colors">
